    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">{{__('welcome.title')}}
                {{--                    <span class="text-muted pt-2 font-size-sm d-block">Javascript array as data source</span></h3>--}}
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{route('admin.welcomes.create')}}" class="btn btn-info font-weight-bolder">
											<span class="svg-icon svg-icon-md">
												<i class="fa fa-plus"></i>
											</span>{{__('welcome.add')}}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.delete').click(function (e) {
                ...
                var url = '{{route('admin.welcomes.destroy')}}';
                var type = "post";
                $.ajax({
                    type: type,
                    url: url,
            ...
            $(function () {
                $('#data-table').DataTable({
                    ...
                    @endif
                    ajax: {
                        url: '{{ route('admin.welcomes.index') }}',
                        // data: function (d) {
                        // }
                    },
                    columnDefs: [
